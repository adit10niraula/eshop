...
const NavBar = () => {
  ...
  const fetchCartAndWishlists = async () => {
    if (user && user.email) {  // Fetch only if user and email are available
      await dispatch(getCartByEmailAsync(user.email));
      await dispatch(fetchUserWishlistAsync(user.email));
    }
  }

  useEffect(() => {
    fetchCartAndWishlists();
  ...
  return (
    <div className='flex flex-col  w-full px-10 py-5 lg:py-0 gap-2  z-40'>
      ...
      <div className=" flex  px-5 max-h-36 flex-row items-center justify-between gap-5">
        {/* Left */}
        <div className="">
          <Link to={'/'} className=' text-3xl flex flex-row items-center gap-2 font-bold'>
            {/* <img src={`/img/logo.png`} alt="" /> */}
            <h1>EShop</h1>
          </Link>
          {
            !navState ? (
              <GiHamburgerMenu className='lg:hidden cursor-pointer w-10 h-10' onClick={() => setnavState(!navState)} />
            ) : (
              <IoCloseCircleOutline className='lg:hidden cursor-pointer w-10 h-10' onClick={() => setnavState(!navState)} />
            )
          }
        </div>
        ...
        <div className="relative">
          <Link to={'/wishlist'}>
            <AiOutlineHeart className=' cursor-pointer w-10 h-10 fill-red-600' />
          </Link>
          {userToken && wishlist?.length > 0 && (
            <p className="absolute  bg-red-600 text-white rounded-[100%] px-2 top-0 -right-1">{wishlist.length}</p>
          )}
        </div>
      </div>
