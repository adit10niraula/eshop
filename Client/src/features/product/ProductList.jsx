...
import { fetchActivityBasedRecommendationAsync, fetchAllProductsAsync, fetchBrandsAsync, fetchCategoryAsync, fetchInteractionBasedRecommendationAsync, fetchProductBySellerIdAsync, fetchProductsByFilterAsync, selectActivityBasedRecommendation, selectAllBrands, selectAllCategories, selectAllProducts } from './productListSlice';
import { sortOptions } from '../../Data/data';
import { Link } from 'react-router-dom';
import { selectLoggedInSeller, selectLoggedInUserToken } from '../Auth/authSlice';
import { createUserInteractionAsync, selectLoggedInUserInfo } from '../user/userSlice';
...
export const ProductList = () => {
  ...
  const activityRecommendations = useSelector(selectActivityBasedRecommendation);

  
  const filteredRecommendations = !activityRecommendations.error
  ? activityRecommendations
      .filter((item) => item.similarity_score >= 0) // Ensure valid similarity scores
      .map((item) => {
        // Create a new object with all product details and `id` set to `product._id.$oid`
        return { ...item.product, id: item.product._id?.$oid };
      })
      .reduce((acc, product) => {
        // Add product only if it doesn't already exist in the accumulator
  ...
  console.log("wishlist", activityRecommendations)
  ...
  const handleAddToWishlist = (id) => {
    ...
    dispatch(createUserInteractionAsync({ productId: id, interactionType: 'added_to_wishlist' }));
  }
          ...
          <main id='ProductList' className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <section aria-labelledby="products-heading" className="pb-24 pt-6">
              ...
              <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                ...
                <div className="lg:col-span-3">
                  ...
                  <ProductGrid products={products} page={page} filters={filters} wishlists={wishlist} handleAddToWishlist={handleAddToWishlist} />
            ...
            <section aria-labelledby="products-heading" className="pb-24 pt-6">
              ...
              <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                ...
                <div className="lg:col-span-3">
                  <Toaster />
                  <h3 id='latestProduct' className="text-4xl font-bold text-center mb-8">Recommended For You</h3>

                  <ProductGrid products={filteredRecommendations} page={1} filters={filters} wishlists={wishlist} handleAddToWishlist={handleAddToWishlist} />
                </div>
              </div>
            </section>
...
function ProductGrid({ products, page, filters, wishlists, handleAddToWishlist }) {
  return (
    <div className="grid grid-cols-2 justify-center lg:grid-cols-3 gap-3">
      {products?.slice(page * 12 - 12, page * 12)?.map((item, index) => {
        const isInWishlist = wishlists.some(wishItem => wishItem.product.id === item.id); // Check if product is in wishlist
        ...
        return (
          <div key={index} className="relative flex flex-col gap-2 lg:items-stretch justify-between px-3 py-4 lg:w-72 shadow-lg">
            ...
            {/* Show filled heart if product is in wishlist, otherwise show outline heart */}
            {isInWishlist ? (
              <GoHeartFill
                className="absolute top-0 right-1 fill-red-600 cursor-pointer w-8 h-8 text-red-500"
              ...
              {item.discountPercentage ? (
                <>
                  ...
                  <p className="text-lg italic font-bold text-red-700">
                    NRS {Math.round(item.price * (1 - item.discountPercentage / 100))}
                  </p>
                </>
              ) : (
